...
<head>
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Archivo:wdth,wght@125,700;125,800;125,900&family=Inter:wght@400;500;600&family=JetBrains+Mono:wght@400;500&display=swap"
        rel="stylesheet" />
    ...
    <div class="relative min-h-screen flex flex-col overflow-x-clip">
            ...
            <nav class="capsule-nav fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[calc(100%-2rem)] max-w-5xl transition-all duration-300">
                <div class="capsule-nav-inner flex items-center gap-2 px-2 py-2">
                    ...
                    <div class="hidden md:flex items-center gap-1 flex-1 px-2">
                        <a href="{{ route('tools.index') }}" class="capsule-nav-link {{ request()->routeIs('tools.*') ? 'active' : '' }}">Tools</a>
                        <a href="{{ route('workflows.index') }}" class="capsule-nav-link {{ request()->routeIs('workflows.*') ? 'active' : '' }}">Workflows</a>
                        <a href="{{ route('pricing') }}" class="capsule-nav-link {{ request()->routeIs('pricing') ? 'active' : '' }}">Pricing</a>
                        <a href="{{ route('blog.index') }}" class="capsule-nav-link {{ request()->routeIs('blog.*') ? 'active' : '' }}">Blog</a>
                    </div>
                    ...
                    <div class="hidden md:flex items-center gap-2 flex-shrink-0">
                        ...
                        @if (Route::has('login'))
                            @auth
                                <x-ui.badge variant="accent" class="gap-1.5 py-1.5 px-3 text-xs">
                            ...
                            @else
                                ...
                                @if (Route::has('register'))
                                    <x-ui.button variant="primary" size="sm" href="{{ route('register') }}" class="capsule-cta-btn">Get Started</x-ui.button>
                                @endif
                            @endauth
                        @endif
                    </div>
        ...
        <footer class="footer-strat py-16 relative mt-24">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                @guest
                    <div
                        class="footer-cta-band mb-16 flex flex-col md:flex-row items-center justify-between gap-8 scroll-reveal">
                        <div>
                            <h3 class="text-2xl font-bold text-text">Ready to automate your channel growth?</h3>
                            <p class="text-text-muted text-sm mt-2 max-w-xl">Create scripts, hooks, and automated workflows
                                in minutes. Scale your content without showing your face.</p>
                        </div>
                ...
                <div class="grid md:grid-cols-4 gap-12 items-start">
                    <div class="col-span-1 md:col-span-2">
                        <div class="flex items-center gap-3 mb-6">
                            <x-application-logo class="h-6 w-auto text-primary" />
                            <span
                                class="font-display font-bold text-lg text-text tracking-tight">{{ \App\Models\Setting::get('site_name', 'AutomateIQ') }}</span>
                        </div>
                        ...
                        <div class="flex gap-3">
                            @if ($twitter = \App\Models\Setting::get('social_twitter'))
                                <a href="{{ $twitter }}"
                                    class="footer-social-icon text-text-muted hover:text-primary transition-colors">
