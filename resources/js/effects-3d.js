...
export function init3DTilt() {
    const tiltElements = document.querySelectorAll('[data-tilt], .card-3d, .tilt-3d');
...
export function initMagneticElements() {
    const magneticElements = document.querySelectorAll('[data-magnetic], .btn-magnetic, .magnetic');
...
export function initScrollAnimations() {
    const animatedElements = document.querySelectorAll('[data-scroll-animate], .scroll-animate');
        ...
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                Array.from(children).forEach((child, index) => {
                    ...
                    child.style.transition = `all 0.6s cubic-bezier(0.16, 1, 0.3, 1) ${index * 100}ms`;
...
export function initTextScramble() {
    ...
    const randomWords = [
        'Innovation', 'Creativity', 'Automation', 'Revolution',
        'Viral Content', 'AI Powered', 'Game Changer', 'Next Level',
        'Pro Creator', 'Go Viral', 'Scale Fast', 'Smart Tools'
    ];
    ...
    class TextScramble {
        ...
        scramble() {
            ...
            const interval = setInterval(() => {
                // Show random word
                this.el.textContent = randomWords[Math.floor(Math.random() * randomWords.length)];
                wordIndex++;
